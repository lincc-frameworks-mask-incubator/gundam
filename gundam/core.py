...
from logging import FileHandler, INFO, StreamHandler, getLogger
...
def findMPIprocs():
    # =========================================================================
    # Try to find out if running with mpirun or standalone
    try:
...
def comdis_worker(z, cosmo):
    """ Worker function for comoving distancesm For more info check the
    documentation
...
def qprint(self):
    ...
    **counts** ouput dictionary or the **par** input dictionary used by Gundam
    ...
    def make_custom_sort(orders):
        ...
        # http://stackoverflow.com/questions/12031482/
        # custom-sorting-python-dictionary
        orders = [
            {k: -i for (i, k) in enumerate(reversed(order), 1)} for order in orders
        ]
    ...
    def eprint3d(m, idn):
        ...
        ellip = " ... "
        txt1 = (
            "[[ " + str(m[0, 0, 0]) + ellip + "///" + ellip + str(m[-1, -1, -1]) + " ]]"
        )
        return "\n".join([txt1])
    ...
    if obj_counts:
        ...
        if k in keys:
            print(
                k.ljust(lj) + "::",
                '"',
                self.logfortran[0 : self.logfortran.find("\n")],
                '... "',
            )
        k = "par"
        if k in keys:
    ...
    if obj_pars:
        ...
        # csort = ['description','kind','estimator','file','file1','outfn',
        #         'h0',
...
def addpvcols(x, cfo, basecolname, **kwargs):
    ...
    basecolname : string
        ...
        for each radial bin
    kwargs :
        Any [key]=value pair to pass to the astropy Column constructor.
        Intended to pass a format specification for the column,
        such as ``format='.4f'``
...
def cnttable(cfi, fmt1=None, fmt2=None, write=False, browser=True):
    ...
    **counts** output dictionary. The table is printed in `stdout` and
    optionally displayed in the default web browser.

    .. rubric:: Parameters

    cfi : string or Munch dictionary
        Filepath for the counts (.cnt) file, or the **counts** dictionary
        itself
    fmt1 : string
        Ouput format of numeric fields (bins, correlations and errors).
        Default='.4f'
    fmt2 : string
        Ouput format of numeric fields (counts). Default='.2f'
    write : bool
        If ``write=True``, save table to disk. Filename will be asked for.
        Default=False
    browser : bool
...
def makebins(nsep, sepmin, dsep, logsep):
    ...
    correlation functions, given the number of bins desired, the minimum bin
    value and the chosen bin width.
    ...
    logsep : bool
        If ``True``, do log-space binning. If ``False``, do linear-space
        binning
    ...
    sepout : float array
        Left, middle and right-side points of each bin. Useful to plot more
        easily
    ...
    .. code-block:: python
        ...
        # Create instead 1 bin of size 50 Mpc, e.g. to work out the pcf
        integrated from 0 to 50 Mpc
    ...
    for i in range(0, nsep + 1):
        if logsep:
            ...
            sep[i] = sepmin + i * dsep
    # For convenience, also return, leftpoint, midpoint and rightpoint of
    # each bin
...
def readcounts(cfile, silent=False):
    ...
    silent : bool
        If False, print a status message indicating the file was read.
        Default=False
...
def plotcf(
    ...
    **kwargs
    ...
    write : bool. Default=False
        Save the figure to disk (default format is pdf). See :ref:
        `Notes <notes-plotcf>` to save in other graphic formats
    figfile : string. Default=None
        Specify an alternative file name for the figure. If specified,
        overrides the default which is to take it from ``par.outfn``. Do not
        add extension.
    ...
    angunit : string. Default='deg'
        ...
        * 'arcmin' : set ouput axis in arcmin (``x`` values are scaled as
                                               ``x``/60)
        * 'deg' : set ouput axis in degrees (``x`` values are scaled as
                                             ``x``/3600)
    ...
    shift : float. Default=0.0
        Fraction of bin size by which ``x`` values are shifted. Useful to
        slightly separate overlapping curves
    ploterrbar : bool. Default=True
        If ``ploterrbar=True``, plot error bars according to ``yerr``
    fill : bool. Default=False
        If ``fill=True``, plot a filled semi-transparent error region instead
        of the usual error bars
    ...
    kwargs : keyword list
        ...
        :func:`pyplot.savefig()`. Use this to customize colors, linestyles,
        markers, etc.
    ...
    * The output graphic format can be changed by passing the ``figformat``
      key in ``kwargs``, e.g. ``figformat='pdf'``. Any format supported by
      matplotlib is valid.
    ...
    cls = (
        ...
        else [kwargs.pop("color")] * 4
    )
    mks = (
        ["o", "s", "^", "d"]
        if "marker" not in kwargs
        else [kwargs.pop("marker")] * len(cls)
    )
    mkss = (
        [4, 4, 4, 4]
        if "markersize" not in kwargs
        else [kwargs.pop("markersize")] * len(cls)
    )
    lin = (
        ["-", "-", "-", "-"]
        if "linestyle" not in kwargs
        else [kwargs.pop("linestyle")] * len(cls)
    )
    lwd = (
        [2, 2, 2, 2]
        if "linewidth" not in kwargs
        else [kwargs.pop("linewidth")] * len(cls)
    )
    # Get alpha for fills
...
def cntplot(cnt, **kwargs):
    """
    Plot a correlation function from a **counts** output dictionary (either
    read from disk or passed directly). Both axes are set to log-space and axes
    labels are selected automatically according to the type of correlation
    (i.e. given by ``par.kind``)
...
def comparecf(
    ...
    clist2 : list of Munch dictionaries / list of strings. Default=None
        ...
        of each correlation curve in ``clist1`` are divided by those in
        ``clist2`` (one-to-one) and plotted in a lower panel. If ``clist2``
        has a single element, the ratios are from all ``clist1`` divided by
        the single ``clist1``.
    ...
    filtneg : bool. Default=False
        ...
        with large errors in a log plot
    label: list
        Optional list of strings to label each correlation function.
        If ommited, the values are taken from the ``outfn`` key stored in the
        count objects
    ...
    color1,marker1,markers1,linestyle1,linewidth1 : lists
        List of colors, marker symbols, marker sizes, line styles and
        line widths
    ...
    (f,ax1,ax2) or (f,ax1) : tuple of handles
        Handles of figure, correlation axis (ax1) and ratio axis (ax2),
        if present
    ...
    .. code-block:: python
        ...
        # Compare one acf on disk with another passed as a counts ouput
        dictionary comparecf(['/proj/galred.cnt', qso])
        # Compare multiple samples and plot sample/control ratios
        f,ax1,ax2 = comparecf(['galred.cnt', 'galblue.cnt', 'galgreen.cnt'],
                              clist2=['allgals.cnt'], fill=True, plotratio=True)
        # Add another curve to previous plot
        comparecf(['qso.cnt'], clist2=['control_qso.cnt'], color2=['k'],
                  f=f, ax1=ax1, ax2=ax2, plotratio=True)
    """
    ...
    # Find out the kind of correlation ---------------
    kind = (
        readcounts(clist1[0]).par.kind if type(clist1[0]) == str else clist1[0].par.kind
    )
    ...
    for i in range(n1):
        ...
        if plotratio and (n2 == 1):
            ...
            if pos2in1 != [i]:
                ...
                plt.xlabel(ax1.get_xlabel())  # get x-axis label as defined by
                                              # plotcf()
...
def fitpowerlaw(
    ...
    **kwargs
    ...
    """Fit a power-law of the form :math:`ax^{\gamma}` to a correlation
    function over a given x coordinate range. Optionally plot the fitted curve
    ...
    kwargs : keyword list
        Any extra [key]=value pairs are passed to :func:
    ...
    out = scipy.optimize.leastsq(errfunc, iguess, args=(xx, yy, yyerr),
                                 full_output=1)
    ...
    if plot:
        ...
        ydom = a * xdom ** gamma
        plt.plot(
            xdom, ydom, color=cls, linestyle=lin, linewidth=lwd, zorder=1,
            **kwargs
        )
        if markfitrange:
            plt.scatter(10 ** xx, 10 ** yy, s=80, facecolors="none",
                        edgecolors="blue")
...
def cntplot2D(
    ...
    **kwargs
    ...
    write : bool. Default=False
        Save the figure to disk (default format is pdf).
        See :ref:`Notes <notes-cntplot2D>` to save in other graphic formats
    figfile : string. Default=None
        Specify an alternative file name for the figure. If ``None``,
        then choose ``cnt.par.outfn`` as default. Do not add extension.
    xlabel, ylabel : string. Default=None
        X-axis and Y-axis labels. If supplied, they override the default labels
    ...
    * The graphic format can be changed by passing the ``figformat`` key in
    ...
    def gaussKern(size):
        ...
        Parameters
        size (int) : kernel size (how many points will be used in the
                                  smoothing operation)
    ...
    # Extend bins
    sepp = makebins(cnt.par.nsepp, cnt.par.seppmin, cnt.par.dsepp,
                    cnt.par.logsepp)[0]
...
def writeasc_cf(lb, mb, rb, f, ferr, par, fmt="%17.5f", altname=None):
    """
    Write an ASCII file for w(rp) / xi(s) / w(th) ouput counts produced by
    the code
...
def writeasc_rppicounts(lb, mb, rb, rppi, par, fmt="%17.5f", cntid=None,
    ...
    par : Munch dictionary
        Used to pass various data, including ``par.outfn`` to name the output
        file
    fmt : string. Default='%17.5f'
        Numeric formatting string
    cntid : string. Default=None
        ID string for column headers. Usually can be 'dd', 'rr', 'dr', etc.
        Also appended as the extension of the ouput file
        (when ``altname=None``)
    altname : string. Default=None
        If supplied, use an alternative file name instead of ``par.outfn`` +
        `.cntid`
    ...
    .. code-block:: python
        ...
        gun.writeasc_rppicounts(c1.rpl, c1.rpm, c1.rpr, c1.dd, c1.par,
                                cntid='dd')
        ...
        #  lb        mb        rb        dd         dd_001    dd_002    ...
        #  0.10000   0.12417   0.14835   11509.00   2082.00   1500.00   ...
        #  0.14835   0.18421   0.22007   20273.00   3122.00   2378.00   ...
        #  0.22007   0.27327   0.32647   36169.00   4940.00   3845.00   ...
        #  0.32647   0.40539   0.48431   64866.00   8453.00   6302.00   ...
        #  ...
    """
    log = getLogger("cf")
...
# =============================================================================
def writeasc_counts(lb, mb, rb, c, par, fmt="%17.5f", cntid=None,
                    altname=None):
    """
    Write an ASCII file for a (1-dimensional) counts array produced by the
    code.
    ...
    .. code-block:: python
        ...
        # Write the DD counts in angular dimensions. Use an alternative file
        name gun.writeasc_counts(c1.thl, c1.thm, c1.thr, c1.dd, c1.par,
                                 cntid='dd', altname='akounts')
...
def tpcf(npt, nrpt, dd, bdd, rr, dr, estimator):
    ...
    zero-valued array with null 2nd dimension,
    e.g. ``bdd=np.zeros([len(dd),0])``
    ...
    See `this paper <http://arxiv.org/pdf/1211.6211v2.pdf>`_ for a nice review
    on estimators and their normalization factors. Here, the normalization
    factors are derived to : (1) keep estimator formulae clean, (2) avoid
    having operations such as (npt*(npt-1)) * dd, where counts are
    multiplied/divided by very big numbers when npt is large.
    ...
    # Loop over bins  ----------------------------------------------
    if estimator == "HAM":
        nf = 4.0 * (npt / (npt - 1)) * (nrpt / (nrpt - 1))  # normalizing
                                                            # factor
...
def tpccf_wrp(npt, nrpt, cd, bcd, cr, dsepv, estimator):
    ...
    estimator : string
        Statistical estimator of the correlation function
            ...
            * 'DP'  : Davis-Peebles -> :math:`CD/CR - 1` (C,D data samples,
                                                          R is random of D)
...
def tpcf_wrp(npt, nrpt, dd, bdd, rr, dr, dsepv, estimator):
    ...
    If boostrap errors are not needed or available, just set ``bdd`` to a
    zero-valued array with null 2nd dimension, e.g.
    ``bdd=np.zeros([len(dd),0])``
    ...
    .. code-block:: python
        ...
        (wrp,wrperr) = tpcf_wrp(npt,nrpt,ddpv,bddpv,rrpv,drpv,dsepv,
                                estimator='HAM')
    """
    fn = sys._getframe().f_code.co_name  # get function self name
    ...
    # Loop over bins  ----------------------------------------------
    if estimator == "HAM":
        nf = 4.0 * (npt / (npt - 1.0)) * (nrpt / (nrpt - 1.0))  # normalizing
                                                                # factor
        for i in range(nsepp):
            twrp = 0
            tbwrp = 0
        ...
        for i in range(nsepp):
            ...
            for j in range(nsepv):
                if rr[i, j] > 0.0:
                    ...
                    for k in range(nbts):
                        bxi[k] = (
                            nf1 * bdd[i, j, k] - nf2 * 2 * dr[i, j] + rr[i, j]
                        ) / rr[i, j]
                else:
                    xi = 0.0  # -1.
                    bxi = np.zeros(nbts)  # - 1.
...
def plotboot(cnt):
    """
    Just a test script to calculate and plot the w(rp) of each bootstrap
    sample, using Landy-Szalay estimator
    """
    npt, nrpt = 1.0 * cnt.npt, 1.0 * cnt.npt1
    nsepp, nsepv, dsepv, nbts = (
        cnt.par.nsepp,
        cnt.par.nsepv,
        cnt.par.dsepv,
        cnt.par.nbts,
...
def packpars(kind="pcf", **kwargs):
    ...
    access (of class ``Munch``). Counting routines rely on this object to
    easily pass multiple parameters at once. For example:
...
def radec2xyz(ra, dec, r=0.5):
    ...
    """
    r = 0.5
    return (r * np.cos(dec) * np.sin(ra), r * np.cos(dec) * np.cos(ra),
            r * np.sin(dec))
...
def bound3d(decs, dcs):
    """
    Return the (maximal) survey boundaries in (RA,DEC,DCOM) for multiple
    samples.
    Note in the RA direction, the limits are **always** set as ramin=0. and
    ramax=360. to make the pair counting valid for surveys that cross the
...
def logtimming(log, cntid, t):
    ...
    t : float
        The time elapsed [s]
    """
    log.info("Done with {} calculations. Looptime (s) : {:0.3f}".format(cntid,
                                                                        t))
    sys.stdout.flush()
...
def setlogs(par, runspyder=True):
    ...
    """
    rank, nranks = findMPIprocs()  # IS WORKING XXXXXX
    if nranks > 1:
        mpiprocstring = "." + str(rank)
    else:
        mpiprocstring = ""

    log = getLogger("cf")  # fix cf name for now
    logfile = os.path.abspath(par.outfn + ".log" + mpiprocstring)
...
def logcallinfo(log, par, npts=[]):
    ...
    npts : list of 1, 2 or 3 integers
        The nr of objects in the input data table, random table, and cross
        sample table
    """
    ...
    log.info(msg)
    msg = "rank         = " + str(par.rank)
    log.info(msg)
    msg = "nranks       = " + str(par.nranks)
    log.info(msg)
    log.info("===============================================================")
...
def initialize(kind, par, nthreads=None, write=None, plot=None):
    ...
    such as reading **par** from disk file if needed, check the parameter
    `kind`, find out if running under Spyder, initialize the logs, etc.
    ...
    write : bool. Default=True
        Flag to generate output count files. Passed just to store it
        under **par**
...
def finalize(log, logf, logff, Ltime, t0, counts):
    ...
    log.info(
        "Loop | Total time (s) ".ljust(27)
        + " : {:0.2f} | {:0.2f}".format(Ltime, t1 - t0)
    )
...
def tidy_counts(tt, par):
    ...
    routines. Basically: (1) Separate dd/bdd, if present, and (2) transpose
    arrays from Fortran optimized indexing back to a more natural row-oriented
    indexing
    ...
    (dd,bdd) : tuple of arrays
        Count and boostrap count arrays. If ``doboot=False``, ``bdd`` is
        set to a zero-valued array with its boostrap samples dimension zeroed
    """
    if par.doboot:
        dd, bdd = tt
    else:
    ...
    # routines return arrays with dimensions exchanged to optimize cache
    # utilization
    if par.kind in ["pcf", "pccf", "rppiA", "rppiC"]:
...
def pixsort(tab, colnms, par):
    ...
    SK pixels and sort the rows of the table according to the pixel index,
    ordered by a given methodology. This way data in a given pixel sits closer
    in memory, largely increasing the efficiency of the cache.
    ...
    par: Munch dictionary
        ...
        (when appropiate), the survey boundaries ``sbound`` and the desired
        method for ordering ``pxorder``. For samples that cross the RA=0 limit,
        it can is useful to specify a custom RA boundary. See :ref:`Custom RA
        boundaries <custRAbound>`
    ...
    """
    if par.kind in ["pcf", "pccf", "rcf", "rccf", "rppiA", "rppiC", "sA",
                    "sC"]:
        ...
        if par.custRAbound is not None:
            ...
            binsra = np.linspace(ramin, ramax, num=par.mxh2)
        pxra = np.digitize(tab[cra].data, bins=binsra)  # opt. add as
                                                        # column tab['pxra']
        binsdec = np.linspace(decmin, decmax, num=par.mxh1)
        pxdec = np.digitize(
            tab[cdec].data, bins=binsdec
        )  # opt. add as column tab['pxdec']
        binsred = np.linspace(zmin, zmax, num=int(par.mxh3))  # num=par.mxh3*10
        pxred = np.digitize(
            tab[cred].data, bins=binsred
        )  # opt. add as column tab['pxred']
        ...
        if par.pxorder == "natural":
            # lexsort() is ~2x faster than argsort(), e.g.
            # sidx=tab.argsort(['pxdec','pxra','pxred'])
            # Note sorting keys must be given in reverse order,
        ...
        # Hilbert order in RA-DEC-Z, using directly the pixel values to sort
            ...
            h = Hilbert(3)
            hilb = list(map(h.decode, list(zip(pxra, pxdec, pxred))))
            sidx = np.argsort(hilb)
            tab["hilb"] = hilb
        ...
        # Optionally add pixel, morton, hilbert numbers to data table.
        # Nice for plotting
        # tab['pxdec'] = pxdec
        # tab['pxra']  = pxra
        # tab['pxred'] = pxred
        ...
        if par.pxorder == "natural":
            # lexsort() is ~2x faster than argsort(), e.g.
            # sidx=tab.argsort(['pxdec','pxra','pxred'])
            # Note sorting keys must be given in reverse order, i.e. from last
            # to first
...
def bestSKgrid2d(par, npts, ras, dens=None):
    """
    Try to find the optimum size (mxh1,mxh2) of a 2D skip grid, for an
    arbitrary sample of (ra,dec) points.
    ...
    #. Find the best ``mxh1`` from an empirical best fit relation of npts vs
    mxh1
    ...
    if dens is None:
        if par.kind in ["thA", "thC"]:  # potentially tune for thC case
            dens = (
                22.0 if npts > 50000 else 16.0
            )  # best times have sort of constant density
    ...
    if par.kind in ["thA", "thC"]:  # potentially tune for thC case
        ...
        h1 = max(int(np.rint(10.75 + 0.075 * np.sqrt(npts))), 1)
...
def bestSKgrid3d(par, npts, ras, dens=None):
    ...
    #. Choose an "optimum" target cell density according to the type of
    correlation
    ...
    if dens is None:
        if par.kind in ["rppiA", "rppiC"]:
            dens = (
                18.0 if npts > 100000 else 8.0
            )  # best times have sort of constant density
        elif par.kind in ["sA", "sC"]:
            dens = (
                28.0 if npts > 100000 else 12.0
            )  # best times have sort of constant density
    ...
    if par.kind in ["rppiA", "rppiC"]:
        ...
        radmax = makebins(par.nseps, par.sepsmin, par.dseps,
                          par.logseps)[0][-1]
    h3 = int((dcmax - dcmin) / radmax)
    ...
    if par.kind in ["rppiA", "rppiC"]:
        ...
        h1 = max(int(np.rint(2.92 + 0.05 * np.sqrt(npts))), 1)
    elif par.kind in ["sA", "sC"]:
        # Based on the fitting of a+b*sqrt(n) to n vs best_mhx1 if sA
        h1 = max(int(np.rint(4.03 + 0.03 * np.sqrt(npts))), 1)
...
def pairs_auto(par, wunit, logff, tab, x, y, z, sk, ll, dc=None):
    ...
    the correct one based on geometry, and choosing the fastest depending
    whether, weights, bootstrap errors, etc. are requested or not.
    ...
    sk,ll : arrays
        Skip table (SK) and linked list table (see :func:`gundam.skll2d` or
                                               :func:`gundam.skll3d`)
...
# =============================================================================
def pairs_cross(
    par, wunit, logff, tab, x, y, z, tab1, x1, y1, z1, sk1, ll1, dc=None,
    dc1=None
):
    """
    ...
    x,y,z,x1,y1,z1 : arrays
        Rectangular coordinates of particles in D table and R rable,
        respectively
    ...
    dc,dc1 : arrays [optional]
        Array of comoving distances of particles in D and R tables.
        Not needed for angular counts
...
def rppi_A(tab, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    nthreads : integer. Default=-1
        Number of threads to use. Default to -1, which means all available
        cpu cores

    write : bool. Default=True
       * True : write several output files for counts, correlations, etc.
       * False : do not write any output files (except for logs, which are
                                                always saved)
    ...
    .. code-block:: python
        ...
        par = gun.packpars(kind='rppiA',outfn='redgalpairs')  # generate
        default parameters
        cnt = gun.rppiA(gals, par, write=True, plot=True)     # get pair counts
        and plot
    """
    lj = 27  # nr of characters for left justification of some status msgs

    # Initialize logs, check if par has the right kind, etc.
    # Common to all CF funcs
    ...
    if par.calcdist:
        ...
        tend = time.time()
        log.info(
            "Comov_dist_tab compute time (s)".ljust(lj)
            + " : {:0.3f}".format(tend - tstart)
        )
    else:
    ...
    log.info("Sample boundaries : (" +
             ("{:0.5f}, " * 6).format(*par.sbound)[:-2] + ")")
    ...
    if par.autogrid:
        ...
        par.mxh1, par.mxh2, par.mxh3, tdens = bestSKgrid3d(
            par, npt, tab[cra].data, dens=par.dens
        )
        log.info("SK cell target density".ljust(lj) +
                 " : {:0.3f}".format(tdens))
    else:
        log.info("SK Autogrid".ljust(lj) + " : OFF")
    log.info(
        "SK grid size [dec,ra,dcom]".ljust(lj)
        + " : "
        + str([par.mxh1, par.mxh2, par.mxh3])
    )
    ...
    tend = time.time()
    log.info(
        "SK-LL tables build time (s)".ljust(lj) +
        " : {:0.3f}".format(tend - tstart)
    )

    # Convert ra,dec,z to spherical coords ------------------------------------
    x, y, z = radec2xyz(tab[cra].data * np.pi / 180.0,
                        tab[cdec].data * np.pi / 180.0)
    ...
    tend = time.time()
    # tend   = MPI.Wtime()
    print("wtime python loop total time", tend - tstart)
    ...
    if plot:
        try:
            ...
            plotcf(seppout[1], intpi, 0, fac=1.0, write=write, par=par,
                   **kwargs)
        except ValueError:
    ...
    counts = buildoutputC(
        par, npts=[npt], binslmr=seppout, dd=dd, bootc=bdd, intpi=intpi,
        intpib=intpib
    )
...
def rppi_C(tab, tab1, par, nthreads=-1, write=True, plot=False, **kwargs):
    """
    Given two astropy data tables, cross-count pairs in the projected-radial
    space (:math:`r_p` vs :math:`\pi` space)
    ...
    .. code-block:: python
        ...
        gals = Table.read('redgal.fits')                       # read data
        par = gun.packpars(kind='rppiC',outfn='qso_rg_pairs')  # generate
        default parameters
        cnt = gun.rppiC(qsos, gals, par, write=True)           # get pair
        counts
    """
    lj = 27  # nr of characters for left justification of some status msgs
    ...
    if par.calcdist:
        ...
        tend = time.time()
        log.info(
            "Comov_dist_tab1 compute time (s)".ljust(lj)
            + " : {:0.3f}".format(tend - tstart)
        )
    else:
        log.info("Using input comov. distances")
    ...
    x, y, z = radec2xyz(tab[cra].data * np.pi / 180.0,
                        tab[cdec].data * np.pi / 180.0)
    x1, y1, z1 = radec2xyz(
        tab1[cra1].data * np.pi / 180.0, tab1[cdec1].data * np.pi / 180.0
    )
    ...
    par.cntid = "DR"
    log.info(
        "====  Counting " + par.cntid + " pairs in " + str(par.mxh1) +
        " DEC bands"
    )
    if runspyder:
        log.info("      [for progress updates check " + logff + "]")
    tstart = time.time()
    tt = pairs_cross(
        par, wunit_dr, logff, tab, x, y, z, tab1, x1, y1, z1, sk1, ll1, dc=dc,
        dc1=dc1
    )
    tend = time.time()
    logtimming(log, par.cntid, tend - tstart)
...
def pcf(tab, tab1, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    nthreads : integer. Default=-1
        Number of threads to use. Default to -1, which means all available cpu
        cores

    write : bool. Default=True
       * True : write several output files for DD/RR/DR counts, correlations,
       etc.
       * False : do not write any output files (except for logs, which are
                                                always saved)
    ...
    .. code-block:: python
        ...
        par = gun.packpars(kind='pcf',outfn='redgal')           # generate
        default parameters
        cnt = gun.pcf(gals, rans, par, write=True, plot=True)   # get pcf and
        plot
    """
    lj = 27  # nr of characters for left justification of some status msgs

    # Initialize logs, check if par has the right kind, etc. Common to all CF
    # functions
    (par, log, logf, logff, runspyder, t0) = initialize(
        "pcf", par, nthreads=nthreads, write=write, plot=plot
    )
    ...
    if par.autogrid:
        ...
        log.info("SK cell target density".ljust(lj) +
                 " : {:0.3f}".format(tdens_dd))
        par_rr.mxh1, par_rr.mxh2, par_rr.mxh3, tdens_rr = bestSKgrid3d(
            par_rr, npt1, tab1[cra1].data, dens=par.dens
        )
        log.info("SK cell target density".ljust(lj) +
                 " : {:0.3f}".format(tdens_rr))
        # For crosscounts choose the grid of randoms. Change if passing
        # Random-Data order instead
        par_dr.mxh1 = par_rr.mxh1
        par_dr.mxh2 = par_rr.mxh2
        par_dr.mxh3 = par_rr.mxh3
        # par_dr.mxh1, par_dr.mxh2, par_dr.mxh3 = par_dd.mxh1, par_dd.mxh2,
        # par_dd.mxh3
    else:
        log.info("SK Autogrid".ljust(lj) + " : OFF")
    log.info(
        "SK grid size [dec,ra,dcom]".ljust(lj)
        + " : "
        + str([par_dd.mxh1, par_dd.mxh2, par_dd.mxh3])
    )
    log.info(
        "SK grid1 size [dec,ra,dcom]".ljust(lj)
        + " : "
        + str([par_rr.mxh1, par_rr.mxh2, par_rr.mxh3])
    )
    ...
    (wrp, wrperr) = tpcf_wrp(
        npt, npt1, dd, bdd, rr, dr, par.dsepv, estimator=par.estimator
    )
    # rpl, rpm, rpr = seppout
    ...
    if plot:
        try:
            ...
            plotcf(seppout[1], wrp, wrperr, fac=1.0, write=write, par=par,
                   **kwargs)
        except ValueError:
            print("Warning: there is a problem with the plot !!!")
...
def pccf(tab, tab1, tab2, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    **cross** samples, this routine calculates the **two-point projected
    ...
    .. code-block:: python
        ...
        par = gun.packpars(kind='pccf',outfn='redgal_qso')      # generate
        default parameters
        cnt = gun.pccf(gals, rans, qsos, par, write=True )      # get pcf
    """
    lj = 27  # nr of characters for left justification of some status msgs

    # Initialize logs, check if par has the right kind, etc. Common to all
    # CF functions
    (par, log, logf, logff, runspyder, t0) = initialize(
    ...
    if par.calcdist:
        ...
        tend = time.time()
        log.info(
            "Comov_dist_tab2 compute time (s)".ljust(lj)
            + " : {:0.3f}".format(tend - tstart)
        )
    else:
        log.info("Using input comov. distances")
    ...
    # Write out the boundary of the survey ------------------------------------
    par.sbound = bound3d(
        [tab[cdec].data, tab1[cdec1].data, tab2[cdec2].data], [dc, dc1, dc2]
    )
    log.info("Sample boundaries : (" +
             ("{:0.5f}, " * 6).format(*par.sbound)[:-2] + ")")
    ...
    if par.autogrid:
        ...
        # We choose to use a single SK grid for all tables, instead of two  
        # different for cd and cr counts. By feeding bestSKgrid3d() with the
        # combination of the 2 largest samples among C,D,R, we get a very
        # good (h1,h2,h3) set. Most likely, its dominated by the random sample
        # R when npt1 is large
        ltn = [npt, npt1, npt2]
        ...
        par.mxh1, par.mxh2, par.mxh3, tdens = bestSKgrid3d(
            par_cr, argn, argras, dens=par.dens
        )
        log.info("SK cell target density".ljust(lj) +
                 " : {:0.3f}".format(tdens))
    ...
    # Sort table/s according to some order  -----------------------------------
    tstart = time.time()
    sidx = pixsort(tab, [cra, cdec, cred], par)  # par or par_cd, par_cr ???
    tab, dc = tab[sidx], dc[sidx]
    sidx1 = pixsort(tab1, [cra1, cdec1, cred1], par)
    tab1, dc1 = tab1[sidx1], dc1[sidx1]
    ...
    # Convert ra,dec,z to spherical coords ------------
    x, y, z = radec2xyz(tab[cra].data * np.pi /
                        180.0, tab[cdec].data * np.pi / 180.0)
    x1, y1, z1 = radec2xyz(
        tab1[cra1].data * np.pi / 180.0,
        tab1[cdec1].data * np.pi / 180.0
    )
    x2, y2, z2 = radec2xyz(
        tab2[cra2].data * np.pi / 180.0,
        tab2[cdec2].data * np.pi / 180.0
    )
    ...
    tstart = time.time()
    tt1 = pairs_cross(
        par_cd, wunit_cd, logff, tab2, x2, y2, z2, tab, x, y, z, sk, ll,
        dc=dc2, dc1=dc
    )
    tend = time.time()
    ...
    # Compute projected correlation function estimate  ------------------------
    (wrp, wrperr) = tpccf_wrp(
        npt, npt1, cd, bcd, cr, par.dsepv, estimator=par.estimator
    )
...
def s_A(tab, par, nthreads=-1, write=True, para=False, plot=False, **kwargs):
    ...
    .. code-block:: python
        ...
        import gundam as gun ; from astropy.table import Table
        gals  = Table.read('redgal.fits')                   # read data
        par = gun.packpars(kind='sA',outfn='redgalpairs')   # generate default
        parameters
        cnt = gun.s_A(gals, par, write=True, plot=True  )   # get counts and
        plot
    """
    lj = 27  # nr of characters for left justification of some status msgs

    # Initialize logs, check if par has the right kind, etc.
    # Common to all CF functions
    (par, log, logf, logff, runspyder, t0) = initialize(
        "sA", par, nthreads=nthreads, write=write, plot=plot
    )
...
# =============================================================================
def s_C(tab, tab1, par, nthreads=-1, write=True, para=False, plot=False,
        **kwargs):
    """
    ...
    .. code-block:: python
        ...
        qsos = Table.read('qso.fits')                        # read data
        gals = Table.read('redgal.fits')                     # read data
        par = gun.packpars(kind='sC',outfn='qso_rg_pairs')   # generate default
        parameters
    ...
    t0 = time.time()
    (par, log, logf, logff, runspyder, t0) = initialize(
        "sC", par, nthreads=nthreads, write=write, plot=plot
    )
    ...
    tstart = time.time()
    tt1 = pairs_cross(
        par, wunit_dr, logff, tab, x, y, z, tab1, x1, y1, z1, sk1, ll1,
        dc=dc, dc1=dc1
    )
    tend = time.time()
    logtimming(log, par.cntid, tend - tstart)
    ...
    # Build ouput COUNTS object  ----------------------------------------------
    counts = buildoutputC(par, npts=[npt, npt1], binslmr=sepsout, dr=dr,
                          bootc=bdr)
...
# =============================================================================
def rcf(tab, tab1, par, nthreads=-1, write=True, para=False, plot=False,
        **kwargs):
    """
    Given two astropy tables corresponding to **data** and **random** samples,
    ...
    .. code-block:: python
        ...
        gals  = Table.read('redgal.fits')                       # read data
        rans  = Table.read('redgal_rand.fits')                  # read randoms
        par = gun.packpars(kind='rcf',outfn='redgal')           # generate
        default parameters
        cnt = gun.rcf(gals, rans, par, write=True, plot=True)   # get rcf and
        plot
    """
    lj = 27  # nr of characters for left justification of some status msgs
    ...
    t0 = time.time()
    (par, log, logf, logff, runspyder, t0) = initialize(
        "rcf", par, nthreads=nthreads, write=write, plot=plot
    )
    ...
    if par.autogrid:
        ...
        log.info("SK cell target density".ljust(lj) +
                 " : {:0.3f}".format(tdens_rr))
        # For crosscounts choose the grid of randoms.
        # Change if passing Random-Data order instead
        par_dr.mxh1 = par_rr.mxh1
        par_dr.mxh2 = par_rr.mxh2
        par_dr.mxh3 = par_rr.mxh3
        # par_dr.mxh1,par_dr.mxh2,par_dr.mxh3 =
        # par_dd.mxh1,par_dd.mxh2,par_dd.mxh3
    else:
        log.info("SK Autogrid".ljust(lj) + " : OFF")
    log.info(
        "SK grid size [dec,ra,dcom]".ljust(lj)
    ...
    if plot:
        try:
            ...
            plotcf(sepsout[1], xis, xiserr, fac=1.0, write=write, par=par,
                   **kwargs)
        except ValueError:
            print("Warning: there is a problem with the plot !!!")
...
def rccf(tab, tab1, tab2, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    **cross** samples, this routine calculates the **two-point redshift space
    ...
    if par.autogrid:
        log.info("SK Autogrid ON")
        # We choose to use a single SK grid for all tables, instead of
        # two different for cd and cr counts. By feeding bestSKgrid3d()
        # with the combination of the 2 largest samples among C,D,R,
        # we get a very good (h1,h2,h3) set. Most likely, its dominated by
        # the random sample R when npt1 is large
        ltn = [npt, npt1, npt2]
        mxposA = np.argmax(ltn)
        ltn[mxposA] = -1
        mxposB = np.argmax(ltn)
    ...
    x2, y2, z2 = radec2xyz(
        tab2[cra2].data * np.pi / 180.0, tab2[cdec2].data * np.pi / 180.0
    )
...
def th_A(tab, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    nthreads : integer. Default=-1
        Number of threads to use. Default to -1, which means all
        available cpu cores

    write : bool. Default=True
       * True : write several output files for counts, correlations, etc.
       * False : do not write any output files (except for logs,
                                                which are always saved)
    ...
    .. code-block:: python
        ...
        import gundam as gun ; from astropy.table import Table
        gals  = Table.read('redgal.fits')                   # read data
        par = gun.packpars(kind='thA',outfn='redgalpairs')  # generate
        default parameters
        cnt = gun.th_A(gals, par, write=True, plot=True  )  # get counts and
        plot
    """
    lj = 27  # nr of characters for left justification of some status msgs
    ...
    # Common to all CF functions
    (par, log, logf, logff, runspyder, t0) = initialize(
        "thA", par, nthreads=nthreads, write=write, plot=plot
    )
    ...
    if par.autogrid:
        log.info("SK Autogrid ON")
        par.mxh1, par.mxh2, tdens = bestSKgrid2d(par, npt, tab[cra].data,
                                                 dens=par.dens)
        log.info("SK cell target density".ljust(lj) +
                 " : {:0.3f}".format(tdens))
    else:
        log.info("Autogrid OFF")
    log.info("SK grid size [dec,ra]".ljust(lj) + " : " +
             str([par.mxh1, par.mxh2]))
    ...
    # Create SK and LL tables  -------------------------------------------
    tstart = time.time()
    sk, ll = cff.mod.skll2d(par.mxh1, par.mxh2, npt, tab[cra], tab[cdec],
                            par.sbound)
    tend = time.time()
    log.info(
        "SK-LL tables build time (s)".ljust(lj) +
...
def th_C(tab, tab1, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    .. code-block:: python
        ...
        qsos = Table.read('qso.fits')                        # read data
        gals = Table.read('redgal.fits')                     # read data
        par = gun.packpars(kind='thC',outfn='qso_rg_pairs')
    ...
    # Common to all CF functions
    (par, log, logf, logff, runspyder, t0) = initialize(
        "thC", par, nthreads=nthreads, write=write, plot=plot
    )
    ...
    # Create SK and LL tables  -------------------------------------------
    tstart = time.time()
    sk1, ll1 = cff.mod.skll2d(
        par.mxh1, par.mxh2, npt1, tab1[cra1], tab1[cdec1], par.sbound
    )
    tend = time.time()
    log.info(
        "SK-LL tables build time (s)".ljust(lj) +
    ...
    par.cntid = "DR"
    log.info(
        "====  Counting " + par.cntid + " pairs in " +
        str(par.mxh1) + " DEC bands"
    )
    if runspyder:
        log.info("      [for progress updates check " + logff + "]")
    tstart = time.time()
    tt = pairs_cross(par, wunit_dr, logff, tab, x, y, z, tab1, x1, y1, z1, sk1,
                     ll1)
    tend = time.time()
    logtimming(log, par.cntid, tend - tstart)
    tacc = tend - tstart
    ...
    # Build ouput COUNTS object  ----------------------------------------------
    counts = buildoutputC(par, npts=[npt, npt1], binslmr=septout, dr=dr,
                          bootc=bdr)
...
def acf(tab, tab1, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    # Common to all CF functions
    (par, log, logf, logff, runspyder, t0) = initialize(
        "acf", par, nthreads=nthreads, write=write, plot=plot
    )
    ...
    if par.autogrid:
        log.info("SK Autogrid ON")
        par_dd.mxh1, par_dd.mxh2, tdens_dd = bestSKgrid2d(
            par_dd, npt, tab[cra].data, dens=par.dens
        )
        log.info("SK cell target density".ljust(lj) +
                 " : {:0.3f}".format(tdens_dd))
        par_rr.mxh1, par_rr.mxh2, tdens_rr = bestSKgrid2d(
            par_rr, npt1, tab1[cra1].data, dens=par.dens
        )
        log.info("SK cell target density".ljust(lj) +
                 " : {:0.3f}".format(tdens_rr))
...
# par_dr.mxh1, par_dr.mxh2, par_dr.mxh3 = par_dd.mxh1, par_dd.mxh2, par_dd.mxh3
        ...
        log.info("Autogrid OFF")
    log.info(
        "SK grid size [dec,ra]".ljust(lj) + " : " +
        str([par_dd.mxh1, par_dd.mxh2])
    )
    log.info(
        "SK grid1 size [dec,ra]".ljust(lj) + " : " +
        str([par_rr.mxh1, par_rr.mxh2])
    )
    ...
    # Create SK and LL tables  -----------------------------------------------
    tstart = time.time()
    sk, ll = cff.mod.skll2d(
        par_dd.mxh1, par_dd.mxh2, npt, tab[cra], tab[cdec], par_dd.sbound
    )
    sk1, ll1 = cff.mod.skll2d(
        par_rr.mxh1, par_rr.mxh2, npt1, tab1[cra1], tab1[cdec1], par_rr.sbound
    )
    tend = time.time()
    log.info(
        "SK-LL tables build time (s)".ljust(lj) +
    ...
    if par.estimator in ("HAM", "DP", "LS"):
        ...
        tstart = time.time()
        tt3 = pairs_cross(
            par_dr, wunit_dr, logff, tab, x, y, z, tab1, x1, y1, z1, sk1, ll1
        )
        tend = time.time()
        logtimming(log, par_dr.cntid, tend - tstart)
        tacc = tacc + (tend - tstart)
    ...
    if plot:
        try:
            ...
            plotcf(septout[1], wth, wtherr, fac=1.0, write=write, par=par,
                   **kwargs)
        except ValueError:
            print("Warning: there is a problem with the plot !!!")
    ...
    #    writeasc_counts(*septout, dd, par, cntid='dd')
    #    if par.estimator not in ('DP'): writeasc_counts(*septout, rr, par,
    #    cntid='rr')
    #    if par.estimator in ('HAM','DP','LS') : writeasc_counts(*septout, dr,
    #    par, cntid='dr')
    #    savepars(par)
    #    savecounts(counts)
...
def accf(tab, tab1, tab2, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    **cross** samples, this routine calculates the **two-point angular space
    ...
    .. code-block:: python
        ...
        rans  = Table.read('redgal_rand.fits')
        qsos  = Table.read('qso.fits')
        par = gun.packpars(kind='accf',outfn='redgal_qso')      # generate
        default parameters
    ...
    if par.autogrid:
        log.info("SK Autogrid ON")
        # We choose to use a single SK grid for all tables, instead of two
        # different for cd and cr counts. By feeding bestSKgrid2d() with the
        # combination of the 2 largest samples among C,D,R, we get a very good
        # (h1,h2,h3) set. Most likely, its dominated by the random sample R
        # when npt1 is large
        ltn = [npt, npt1, npt2]
        mxposA = np.argmax(ltn)
        ltn[mxposA] = -1
        ...
        argn = [ltn[i] for i in [mxposA, mxposB]]
        argras = [ltras[i] for i in [mxposA, mxposB]]
        par.mxh1, par.mxh2, tdens = bestSKgrid2d(par_cr, argn, argras,
                                                 dens=par.dens)
    ...
    tend = time.time()
    log.info(
        "SK-LL tables build time (s)".ljust(lj) + " : {:0.3f}".format(tend
                                                                      - tstart)
    )

    # Convert ra,dec,z to spherical coords ------------
    x, y, z = radec2xyz(tab[cra].data * np.pi / 180.0, tab[cdec].data * np.pi
                        / 180.0)
    x1, y1, z1 = radec2xyz(
        tab1[cra1].data * np.pi / 180.0, tab1[cdec1].data * np.pi / 180.0
    )
    ...
    if runspyder:
        log.info("      [for progress updates check " + logff + "]")
    tstart = time.time()
    tt1 = pairs_cross(par_cd, wunit_cd, logff, tab2, x2, y2, z2, tab, x, y, z,
                      sk, ll)
    tend = time.time()
    logtimming(log, par_cd.cntid, tend - tstart)
    tacc = tend - tstart
    ...
    tstart = time.time()
    tt2 = pairs_cross(
        par_cr, wunit_cr, logff, tab2, x2, y2, z2, tab1, x1, y1, z1, sk1, ll1
    )
    tend = time.time()
    logtimming(log, par_cd.cntid, tend - tstart)
    tacc = tacc + (tend - tstart)
...
# =============================================================================
def buildoutputC(
    par, npts=[], binslmr=[], dd=None, dr=None, bootc=None, intpi=None,
    intpib=None
):
    """
    ...
    This is for the main Gundam functions that calculate a single pair count,
    i.e. :func:`gundam.rppi_A`, :func:`gundam.rppi_C`, :func:`gundam.s_A`,
    ...
    npts : list of int. Default=[]
        ...
        example npts=[400, 4000] in cross-count cases; npts=[400] for
        auto-count cases
...
def buildoutput(
    ...
    This is for the main Gundam functions that calculate a multiple pair
    counts, i.e. :func:`gundam.pcf`, :func:`gundam.pccf`, :func:`gundam.rcf`,
