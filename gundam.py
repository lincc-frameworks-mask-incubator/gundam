...
# =============================================================================
def plotcf(x, y , yerr, fac=1., write=False, par=None, angunit='deg', 
    ...
    write : bool. Default=False
        If ``write=True``, save the figure to disk (default format is eps). See
        :ref:`Notes <notes-plotcf>` to save in other graphic formats
    ...
    kwargs : keyword list
        ...
        :func:`matplotlib.pyplot.plot()` routine, except for the transparency level 
        ``alpha`` which is passed to :func:`pyplot.fill_between()`. Use this to 
        customize colors, linestyles, markers, etc.
    ...
    * The output graphic format can be changed by passing the ``figformat`` key in 
    ...
    # Get graphic format
    figformat = 'eps'  if 'figformat' not in kwargs else kwargs.pop('figformat')
    ...
    if ( (np.shape(yerr) != ()) or ((y > 0.).any()) ) and (ploterrbar):
        if not(fill):
            plt.errorbar(x, y, yerr=yerr, fmt='none', ecolor=cls[cps],elinewidth=lwd[cps],label=None)
        else:
    ...
    if (write) and (par != None):
        plt.savefig(par.outfn+'.'+par.kind+'.'+figformat,format=figformat)
...
# =============================================================================
def cntplot2D(cnt, estimator=None, slevel=5, write=False, xlabel=None, ylabel=None, 
              cmap='jet',**kwargs):
    """
    ...
    write : bool. Default=False
        If ``write=True``, save the figure to disk (default format is eps). See
        :ref:`Notes <notes-cntplot2D>` to save in other graphic formats
    xlabel, ylabel : string. Default=None
        X-axis and Y-axis labels. If supplied, they override the default labels
    ...
    * The graphic format can be changed by passing the ``figformat`` key in 
    ...
    plt.contour(x[0:-1,0:-1],y[0:-1,0:-1],qqs,levels=lev,colors='k',linestyles='solid')
    # Plot titles
    ...
    if write :
        plt.savefig(cnt.par.outfn+'.'+'.2DCF.'+figformat, format=figformat)
    ...
    if estimator == 'DP':
        ...
        for i in range(nseps):        
            if cr[i] > 0. :
                xi = (cd[i]/cr[i])*sf - 1.
                for j in range(nbts):
                    bxi[j] = (bcd[i,j]/cr[i])*sf - 1.
                #xierr = np.std(bxi)
                xierr = np.std(bxi) if nbts>0 else 0.
            else:
                xi    = -1.
                xierr = -1.
            fxi[i]    = xi
...
def tpccf_wrp(npt,nrpt,cd,bcd,cr,dsepv,estimator):
    ...
    # Collapse all radial bins  ---------------------------
    #collapdsepv = dsepv*cd.shape[1]    # Width of single radial bin (will not work in log bins)
    #cd = cd.sum(axis=1).reshape(-1,1)  # Collapse along pi direction 
    #cr = cr.sum(axis=1).reshape(-1,1)  # Collapse along pi direction
    #if nbts>0 :
    #    bcd = bcd.sum(axis=1).reshape(-1,1,nbts)  #collapse along pi direction
    #else:
    #    bcd = 0.                       # Dummy since will not be used anyway
    ...
    #    for i in range(nsepp):
    #        if cr[i,0] > 0. :
    #            xi = (cd[i,0]/cr[i,0])*sf - 1.
    #            for k in range(nbts):
    #                bxi[k] = (bcd[i,0,k]/cr[i,0])*sf - 1.
    #        else:
    #            xi = -1.
    #            bxi = np.zeros(nbts) - 1.
    #        wrp[i] = xi*dsepv
    #        if nbts>0 : wrperr[i] = np.std(bxi*dsepv)
    return (wrp,wrperr)
...
def packpars(kind='pcf',**kwargs):
    ...
    * par=packpars(kind='pcf',nsepp=12) : 
    ...
    if kind == 'sA' or kind == 'sC':
        p.h0       = 100.                # Hubble constant [km/s/Mpc]
    ...
    if kind == 'rppiA' or kind == 'rppiC':
        ...
        p.dsepv    = 40.                 # Bin size in pi [Mpc/h]
        p.calcdist = True                # Calculate comoving distances
        p.file     = ''                  # Filename of input data catalog
        p.description = ''               # Description of CF run
        p.cra      = 'ra'                # Colunm name of RA coordinate [deg]
    ...
    for key,value in kwargs.items():
        p.update({key:value})
    if 'outfn' not in kwargs: print('Missing outfn: default to',p.outfn)
...
def bound3d(decs, dcs):
    ...
    ramin  = 0.
    ramax  = 360.
    decmin = max(min([min(d) for d in decs]) - delta, -90.)
    decmax = min(max([max(d) for d in decs]) + delta, 90.)
    dcmin  = max(min([min(dc) for dc in dcs]) - delta, 0.)
...
def logtimming(log, cntid, t):
    ...
    t : float
        The time elapsed [s]
    """
    log.info("Done with {} calculations. Looptime (s) : {:0.3f}".format(cntid,t))
    sys.stdout.flush()
...
def setlogs(par, runspyder=True):
    ...
    if runspyder: log.addHandler(StreamHandler(sys.stdout))  #add only under Spyder
    log.setLevel(INFO)
    
    # Delete the Fortran log file (to avoid being appended in succesive runs)
    logfort = par.outfn + '.fortran.log'
    if os.path.isfile(logfort) :  os.remove(logfort)
...
def check_kind(par, kind):
    ...
    if par.kind != kind : raise Exception(msg)
...
def logcallinfo(log, par, npts=[]):
    ...
    if len(npts)>0:
        msg = 'table        = ' + par.file        ;  log.info(msg)
        msg = 'table_nrows  = ' + str(npts[0])    ;  log.info(msg)
    if len(npts)>1:
        msg = 'table1       = ' + par.file1       ;  log.info(msg)
        msg = 'table1_nrows = ' + str(npts[1])    ;  log.info(msg)
    if len(npts)>2:
        msg = 'table2       = ' + par.file2       ;  log.info(msg)
        msg = 'table2_nrows = ' + str(npts[2])    ;  log.info(msg)
    msg = 'nthreads     = ' + str(par.nthreads) ;  log.info(msg)
    msg = 'write        = ' + str(par.write)    ;  log.info(msg)
    msg = 'plot         = ' + str(par.plot)     ;  log.info(msg)
    log.info('=====================================================================')
...
def initialize(kind, par, nthreads=None, write=None, plot=None):
    ...
    if type(par) is str:  par = readpars(par)
...
def finalize(log, logf, logff, Ltime, t0, counts):
    ...
    log.info('Loop | Total time (s) '.ljust(27)+' : {:0.2f} | {:0.2f}'.format(Ltime,t1-t0))
...
def tidy_counts(tt, par):
    ...
    if par.doboot:
        ...
        dd      = tt
        if par.kind in ['pcf','pccf','rppiA','rppiC']:  bdd = np.zeros([0,par.nsepv,par.nsepp])
        if par.kind in ['acf','accf','thA','thC']:  bdd = np.zeros([0,par.nsept])
        if par.kind in ['rcf','rccf','sA','sC']:  bdd = np.zeros([0,par.nseps])
...
def pixsort(tab, colnms, par):
    ...
    if par.kind in ['pcf','pccf','rcf','rccf','rppiA','rppiC','sA','sC']:
        ...
        # large(?) integer
        elif par.pxorder is 'morton_dir':
            import mortcode as mc
            lon = tab['RA'].data  ;  lat = tab['DEC'].data
            if min(lat) < 0.0 : lat = lat-min(lat)
...
def bestSKgrid2d(par, npts, ras, dens=None):
    ...
    This is far from trivial, so for the moment, this routine just gets ``mxh1``
    from an empirical best fit relation of npts vs mxh1, and then adjusting 
    ``mxh2`` to reach a given cell density
    ...
    npts : integer or list of integers
        Nr. of objects in the sample(s). For cross-correlations the largest
        samples size is adopted as the effective ``npts`` (xxxxx ok?)
    ...
    if par.kind in ['thC'] : 
        ...
        npts = max(npts)           # In DR case, choose the largest among [sizeD,sizeR]
                                   # Hence will work only if R is the 2nd sample passed
                                   # to rppiC()

    # Set default density based on the best nr.part.per.cell as function of 
    # sample size, as found in timming grids. This is mostly constant for large
    # samples and goes down a bit in small samples
    if dens is None:
        if par.kind == 'thA':
            dens = 22. if npts>50000 else 18. #15.1475 + 1.9098e-04*npts  # TODO!
        if par.kind == 'thC':
            dens = 22.   # TODO !
    ...
    # Choose mxh1  ------------------------------
    if par.kind == 'thA' :
        # Based on the fitting of a+b*sqrt(n) to n vs best_mhx1 
        h1 = max( np.int(np.rint(-6.23744777 + 0.11568223*np.sqrt(npts))), 1)
    if par.kind == 'thC' :              ### TODO FITTING  !!!
        # Based on the fitting of a+b*sqrt(n) to n vs best_mhx1 
        h1 = max( np.int(np.rint(0.760572 + 0.056109*np.sqrt(npts))), 1)

    # Choose mxh2  ------------------------------
    # Set to reach the target density (last factor is to extend to 360 range)
    h2 = np.int(np.rint(h1h2/h1)*(360./samplewidth))
...
def bestSKgrid3d(par, npts, ras, dens=None):
    ...
    dens : float
        The effective cell density adopted
    """
    if par.kind in ['rppiC','sC','pccf'] : 
        ras = np.concatenate(ras)  # Flatten the list of two arrays if needed
        npts = sum(npts)           # Sum the size of both samples
                                   
    # Set default density based on the best nr.part.per.cell as function of 
    # sample size, as found in timming grids. This is not constant (why?)
    # for large samples, even while we already maxed out the 8MB L3 cache
    if dens is None:
        if par.kind == 'rppiA': dens = 12.6774 + 0.01186*np.sqrt(npts)
        if par.kind == 'rppiC': dens = 12.6774 + 0.01186*npts          # TODO!! fit not real
        if par.kind == 'sA':    dens = 12.6774 + 0.01186*np.sqrt(npts) # TODO!! fit not real
        if par.kind == 'sC':    dens = 12.6774 + 0.01186*npts          # TODO!! fit not real
    #print('SK cell target density : ', dens)
    ...
    if par.kind in ['rppiA','rppiC']:
        radmax = makebins(par.nsepv,0.,par.dsepv,0)[0][-1]
    if par.kind in ['sA','sC']:
    ...
    if par.kind == 'rppiA':
        # Based on the fitting of a+b*sqrt(n) to n vs best_mhx1 
        h1 = max( np.int(np.rint(19.0703 + 0.00002565*np.sqrt(npts))), 1)
        #h1 = max( np.int(np.rint(-0.84610468 + 0.07588521*np.sqrt(npts))), 1)
    if par.kind == 'rppiC':                                ### TODO FITTING  !!!
        # Based on the fitting of a+b*sqrt(n) to n vs best_mhx1 
        h1 = max( np.int(np.rint(-1.1778 + 0.02952*np.sqrt(npts))), 1)
    if par.kind == 'sA':                                   ### TODO FITTING  !!!
        # Based on the fitting of a+b*sqrt(n) to n vs best_mhx1 
        h1 = max( np.int(np.rint(1.1744 + 0.02108*np.sqrt(npts))), 1)
    if par.kind == 'sC':                                   ### TODO FITTING  !!!
        # Based on the fitting of a+b*sqrt(n) to n vs best_mhx1 
        h1 = max( np.int(np.rint(-1.1778 + 0.02952*np.sqrt(npts))), 1)
...
def pairs_auto(par, wunit, logff, tab, x, y, z, sk, ll, dc=None):
    ...
    if par.kind in ('pcf','rppiA'):  ###############  PCF Case   ##############
        ...
        if par.doboot:     # Counting Data + Boostrap
            if (par.wfib==False and wunit):
                args = [nt,npt,tab[par.cdec].data,dc,x,y,z,par.nsepp,sepp,par.nsepv,sepv,par.sbound,par.mxh1,par.mxh2,par.mxh3,par.nbts,par.bseed,par.cntid,logff,sk,ll]
            ...
            if (par.wfib==False and wunit):
                args = [nt,npt,tab[par.cdec].data,dc,x,y,z,par.nsepp,sepp,par.nsepv,sepv,par.sbound,par.mxh1,par.mxh2,par.mxh3,par.cntid,logff,sk,ll]
    ...
    if par.kind in ('acf','thA'):  ###############  ACF Case   ################
        sept= makebins(par.nsept, par.septmin, par.dsept, par.logsept)[0]
        if par.doboot:     # Counting Data + Boostrap
            if (par.wfib==False and wunit):
                args = [nt,npt,tab[par.cdec].data,x,y,z,par.nsept,sept,par.sbound,par.mxh1,par.mxh2,par.nbts,par.bseed,par.cntid,logff,sk,ll]
                tt   = cff.mod.th_Ab(*args)        # fast unweighted counting
            else:
                args = [nt,npt,tab[par.cdec].data,tab[par.cwei],x,y,z,par.nsept,sept,par.sbound,par.mxh1,par.mxh2,par.nbts,par.bseed,par.wfib,par.cntid,logff,sk,ll]
                tt   = cff.mod.th_Ab_wg(*args)     # weighted counting
        else:              # Counting Data only
            if (par.wfib==False and wunit):
                args = [nt,npt,tab[par.cdec].data,x,y,z,par.nsept,sept,par.sbound,par.mxh1,par.mxh2,par.cntid,logff,sk,ll]
                tt   = cff.mod.th_A(*args)         # fast unweighted counting
            else:
                args = [nt,npt,tab[par.cdec].data,tab[par.cwei],x,y,z,par.nsept,sept,par.sbound,par.mxh1,par.mxh2,par.wfib,par.cntid,logff,sk,ll]
...
def pairs_cross(par, wunit, logff, tab, x, y, z, tab1, x1, y1, z1, sk1, ll1, dc=None, dc1=None):
    ...
    if par.kind in ('pccf','rppiC'):  ###############  PCF Case   ##############
        sepp = makebins(par.nsepp, par.seppmin, par.dsepp, par.logsepp)[0]
        sepv = makebins(par.nsepv, 0., par.dsepv, False)[0]
        if par.doboot:     # Counting Data + Boostrap
            if (par.wfib==False and wunit):
                args = [nt,npt,tab[par.cra].data,tab[par.cdec].data,dc,x,y,z,npt1,dc1,x1,y1,z1,par.nsepp,sepp,par.nsepv,sepv,par.sbound,par.mxh1,par.mxh2,par.mxh3,par.nbts,par.bseed,par.cntid,logff,sk1,ll1]
            ...
            if (par.wfib==False and wunit):
                args = [nt,npt,tab[par.cra].data,tab[par.cdec].data,dc,x,y,z,npt1,dc1,x1,y1,z1,par.nsepp,sepp,par.nsepv,sepv,par.sbound,par.mxh1,par.mxh2,par.mxh3,par.cntid,logff,sk1,ll1]
    ...
    if par.kind in ('accf','thC'):  ###############  ACF Case   ################
        sept= makebins(par.nsept, par.septmin, par.dsept, par.logsept)[0]
        if par.doboot:     # Counting Data + Boostrap
            if (par.wfib==False and wunit):
                args = [nt,npt,tab[par.cra].data,tab[par.cdec].data,x,y,z,npt1,x1,y1,z1,par.nsept,sept,par.sbound,par.mxh1,par.mxh2,par.nbts,par.bseed,par.cntid,logff,sk1,ll1]
                tt   = cff.mod.th_Cb(*args)        # fast unweighted counting
            else:
                args =  [nt,npt,tab[par.cra].data,tab[par.cdec].data,tab[par.cwei].data,x,y,z,npt1,tab1[par.cwei1].data,x1,y1,z1,par.nsept,sept,par.sbound,par.mxh1,par.mxh2,par.nbts,par.bseed,par.wfib,par.cntid,logff,sk1,ll1]
                tt   = cff.mod.th_Cb_wg(*args)     # weighted counting
        else:              # Counting Data only
            if (par.wfib==False and wunit):
                args = [nt,npt,tab[par.cra].data,tab[par.cdec].data,x,y,z,npt1,x1,y1,z1,par.nsept,sept,par.sbound,par.mxh1,par.mxh2,par.cntid,logff,sk1,ll1]
                tt   = cff.mod.th_C(*args)         # fast unweighted counting
            else:
                args = [nt,npt,tab[par.cra].data,tab[par.cdec].data,tab[par.cwei],x,y,z,npt1,tab1[par.cwei1],x1,y1,z1,par.nsept,sept,par.sbound,par.mxh1,par.mxh2,par.wfib,par.cntid,logff,sk1,ll1]
...
def rppi_A(tab, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    (par, log, logf, logff, runspyder, t0) = initialize('rppiA', par, nthreads=nthreads, write=write, plot=plot)
    ...
    sepv, sepvout = makebins(par.nsepv, 0., par.dsepv, False)
    ...
    tstart = time.time()
    sk,ll  = cff.mod.skll3d(par.mxh1, par.mxh2, par.mxh3, npt, tab[cra], tab[cdec],
                            dc, par.sbound, sepv, par.nsepv)
    tend   = time.time()
    ...
    if plot:
        try:
            ...
            plotcf(seppout[1], intpi, 0, fac=1., write=write, par=par, **kwargs)
        except ValueError:
            print('Warning: there is a problem with the plot !!!') 

    # Build ouput COUNTS object  ----------------------------------------------
    counts = buildoutputC(par, npts=[npt], binslmr=seppout, dd=dd, bootc=bdd,
                          intpi=intpi, intpib=intpib)
...
def rppi_C(tab, tab1, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    # Initialize logs, check if par has the right kind, etc. Common to all CF funcs
    (par,log,logf,logff,runspyder,t0) = initialize('rppiC', par, nthreads=nthreads, write=write, plot=plot)
    ...
    if par.autogrid:
        log.info('SK Autogrid'.ljust(lj) + ' : ON')
        par.mxh1,par.mxh2,par.mxh3,tdens = bestSKgrid3d(par,[npt,npt1],[tab[cra].data,tab1[cra1].data],dens=par.dens)
        log.info('SK cell target density'.ljust(lj)+' : {:0.3f}'.format(tdens))
    else:
        log.info('SK Autogrid'.ljust(lj) + ' : OFF')
    log.info('SK grid size [dec,ra,dcom]'.ljust(lj)+' : ' + str([par.mxh1,par.mxh2,par.mxh3]))
    ...
    tstart = time.time()
    sidx   = pixsort(tab,[cra, cdec, cred],par)
    tab    = tab[sidx] ;  dc = dc[sidx]
    sidx1  = pixsort(tab1,[cra1, cdec1, cred1],par)
    tab1   = tab1[sidx1] ;  dc1 = dc1[sidx1]
    tend   = time.time()
    log.info('Pixsort time (s)'.ljust(lj)+' : {:0.3f}'.format(tend-tstart))
    
    # Create SK and LL tables  -------------------------------------------
    tstart  = time.time()
    sk1,ll1 = cff.mod.skll3d(par.mxh1, par.mxh2, par.mxh3, npt1, tab1[cra1], tab1[cdec1], 
                             dc1, par.sbound, sepv, par.nsepv)
    tend    = time.time()
    log.info('SK-LL tables build time (s)'.ljust(lj)+' : {:0.3f}'.format(tend-tstart))
    ...
    if runspyder : log.info('      [for progress updates check ' + logff + ']')
    tstart = time.time()
    tt     = pairs_cross(par, wunit_dr, logff, tab, x, y, z, 
                         tab1, x1, y1, z1, sk1, ll1,  dc=dc, dc1=dc1)
    tend   = time.time()
    logtimming(log, par.cntid, tend-tstart)
    tacc = tend-tstart
    #=========================  END PAIR COUNTS  ==============================
    ...
    # Build ouput COUNTS object  ----------------------------------------------
    counts = buildoutputC(par,npts=[npt,npt1],binslmr=seppout,dr=dr,bootc=bdr,
                          intpi=intpi,intpib=intpib)
...
def pcf(tab, tab1, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    if par.autogrid:
        log.info('SK Autogrid'.ljust(lj) + ' : ON')
        par_dd.mxh1, par_dd.mxh2, par_dd.mxh3, tdens_dd = bestSKgrid3d(par_dd,npt,tab[cra].data,dens=par.dens)
        log.info('SK cell target density'.ljust(lj)+' : {:0.3f}'.format(tdens_dd))
        par_rr.mxh1, par_rr.mxh2, par_rr.mxh3, tdens_rr = bestSKgrid3d(par_rr,npt1,tab1[cra1].data,dens=par.dens)
        log.info('SK cell target density'.ljust(lj)+' : {:0.3f}'.format(tdens_rr))
        # Suitable for Random-Data order in crosscounts. Change if passing Data-Random order
    ...
    # Create SK and LL tables  -----------------------------------------------
    tstart = time.time()
    sk,ll   = cff.mod.skll3d(par_dd.mxh1,par_dd.mxh2,par_dd.mxh3,npt,tab[cra],tab[cdec],dc,par_dd.sbound,sepv,par_dd.nsepv)
    sk1,ll1 = cff.mod.skll3d(par_rr.mxh1,par_rr.mxh2,par_rr.mxh3,npt1,tab1[cra1],tab1[cdec1],dc1,par_rr.sbound,sepv,par_rr.nsepv)
    tend = time.time()
    log.info('SK-LL tables build time (s)'.ljust(lj)+' : {:0.3f}'.format(tend-tstart))
    ...
    if par.estimator in ('HAM','DP','LS'):
        ...
        if runspyder : log.info('      [for progress updates check ' + logff + ']')
        tstart = time.time()
        tt3    = pairs_cross(par_dr, wunit_dr, logff, tab, x, y, z, 
                             tab1, x1, y1, z1, sk1, ll1,  dc=dc, dc1=dc1)
        tend   = time.time()
    ...
    rr,dum = tidy_counts(tt2, par_rr) if par.estimator not in ('DP') else (np.zeros([par.nsepp,par.nsepv]),0)
    dr,dum = tidy_counts(tt3, par_dr) if par.estimator in ('HAM','DP','LS') else (np.zeros([par.nsepp,par.nsepv]),0)
    ...
    if plot:
        try:
            ...
            plotcf(seppout[1], wrp, wrperr, fac=1., write=write, par=par, **kwargs)
        except ValueError:
            print('Warning: there is a problem with the plot !!!') 

    # Build ouput COUNTS object  ----------------------------------------------
    counts = buildoutput(par, npts=[npt,npt1], binslmr=seppout, dd=dd, rr=rr, dr=dr,
                         bootc=bdd, cf=wrp, cferr=wrperr)
    ...
    if write:
        ...
        if par.estimator not in ('DP') : writeasc_rppicounts(*seppout, rr, par, cntid='rr')
        if par.estimator in ('HAM','DP','LS') : writeasc_rppicounts(*seppout, dr, par, cntid='dr')
        savepars(par)
        savecounts(counts)
...
def pccf(tab, tab1, tab2, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    **cross** samples, this routine calculates the **two-point projected 
    ...
    # Initialize logs, check if par has the right kind, etc. Common to all CF functions
    (par,log,logf,logff,runspyder, t0) = initialize('pccf', par, nthreads=nthreads, write=write, plot=plot)
    ...
    sk,ll   = cff.mod.skll3d(par.mxh1,par.mxh2,par.mxh3,npt,tab[cra],tab[cdec],dc,par.sbound,sepv,par.nsepv)
    sk1,ll1 = cff.mod.skll3d(par.mxh1,par.mxh2,par.mxh3,npt1,tab1[cra1],tab1[cdec1],dc1,par.sbound,sepv,par.nsepv)
    tend    = time.time()
    log.info('SK-LL tables build time (s)'.ljust(lj)+' : {:0.3f}'.format(tend-tstart))
    ...
    # Build ouput  ------------------------------------------------------------
    counts = buildoutput(par, npts=[npt,npt1,npt2], binslmr=seppout, cd=cd, cr=cr,
                         bootc=bcd, cf=wrp, cferr=wrperr)
...
# =============================================================================
def s_A(tab, par, write=True, para=False, plot=False):
    """
    ...
    # Initialize logs, check if par has the right kind, etc. Common to all CF functions
    t0 = time.time()
    (par,log,logf,logff,runspyder) = initialize('sA', par, para=para, write=write, plot=plot)
    ...
    # If requested, try to find the best SK grid for DD counts  ---------------
    if par.autogrid:
        log.info('SK Autogrid ON')
        par.mxh1,par.mxh2,par.mxh3,tdens = bestSKgrid3d(par,npt,tab[cra].data,dens=par.dens)
        log.info('SK cell target density'.ljust(lj)+' : {:0.3f}'.format(tdens))
    ...
    #==========================   COUNT PAIRS   ===============================
    par.cntid  = 'DD'
    log.info('====  Counting ' + par.cntid + ' pairs =====')
    ...
    if write:
        writeasc_counts(sepsout[0],sepsout[1],sepsout[2],dd,par,cntid='dd')
    ...
    if plot:
        try:
            #plt.figure(1)
            plt.figure('Counts plot')
            plotcf(sepsout[1],dd,0,fac=1.,write=write,par=par)
        except ValueError:
            print('Warning: there is a problem with the plot !!!') 
...
# =============================================================================
def s_C(tab, tab1, par, write=True, para=False, plot=False):
    """
    ...
    # Initialize logs, check if par has the right kind, etc. Common to all CF functions
    t0 = time.time()
    (par,log,logf,logff,runspyder) = initialize('sC', par, para=para, write=write, plot=plot)
    ...
    if par.para:
        ...
        def comdis(z):
            # Wrapper for astropy's comoving_distance() method, decorated to run in
            # parallel as a local function. Since we need dv, this must go after 
            # init() and inside par_pcf(), par_pccf(), etc.
            return cosmo.comoving_distance(z).value
    else:
        def comdis(z):
            # Wrapper for astropy's comoving_distance() method
    ...
    # Write out the boundary of the survey -------------
    par.sbound = bound3d([tab[cdec].data,tab1[cdec1].data],[dc,dc1])
    log.info('Boundaries = ' + str(par.sbound))
    ...
    # If requested, try to find the best SK grid size  ------------------------
    if par.autogrid:
        log.info('Autogrid ON')
        par.mxh1,par.mxh2,par.mxh3,tdens = bestSKgrid3d(par,[npt,npt1],[tab[cra].data,tab1[cra1].data],dens=par.dens)
        log.info('SK cell target density'.ljust(lj)+' : {:0.3f}'.format(tdens))
    else:
        log.info('Autogrid OFF')
    log.info('SK grid [dec,ra,dcom] = ' + str([par.mxh1,par.mxh2,par.mxh3]))
    ...
    allw_dr = allw and allw1

    #==========================================================================
    #==========================   COUNT PAIRS   ===============================
    log.info('====  Counting CROSS pairs =====')
    par.cntid  = 'DR'
    ...
    if par.para is True:
        ...
        tstart = time.time()
        #if rc.dbmode: do something different?
        tt1 = v.map_async(fcall_sC, ixseq)
        watch(tt1, rc, tolog=True, log=log)
        tend = time.time()
    ...
    # Aggregate counts  -------------------------------------------------------
    dr,bdr = aggcountsb(tt1,par)
    
    # Write ascii counts, correlations and parameters  ------------------------
    if write:
        writeasc_counts(sepsout[0],sepsout[1],sepsout[2],dr,par,cntid='dr')
        # Write log entry now just for order  -------
        msg = '> COUNTS object saved in  : ' + par.outfn + '.cnt'
        log.info(msg)
    ...
    if plot:
        try:
            #plt.figure(1)
            plt.figure('Counts plot')
            plotcf(sepsout[1],dr,0,fac=1.,write=write,par=par)
        except ValueError:
            print('Warning: there is a problem with the plot !!!') 
...
# =============================================================================
def rcf(tab, tab1, par, write=True, para=False, plot=False):
    """
    ...
    # Initialize logs, check if par has the right kind, etc. Common to all CF functions
    t0 = time.time()
    (par,log,logf,logff,runspyder) = initialize('rcf', par, para=para, write=write, plot=plot)
    ...
    # Create SK and LL tables  -----------------------------------------------
    tstart = time.time()
    sk,ll   = cff.mod.skll3d(par_dd.mxh1,par_dd.mxh2,par_dd.mxh3,npt,tab[cra],tab[cdec],dc,par_dd.sbound,seps,par_dd.nseps)
    sk1,ll1 = cff.mod.skll3d(par_rr.mxh1,par_rr.mxh2,par_rr.mxh3,npt1,tab1[cra1],tab1[cdec1],dc1,par_rr.sbound,seps,par_rr.nseps)
    tend = time.time()
    log.info('SK-LL tables build time (s)'.ljust(lj)+' : {:0.3f}'.format(tend-tstart))
    ...
    if write:
        ...
        writeasc_counts(sl,sm,sr,dd,par,cntid='dd')
        if par.estimator not in ('DP'): writeasc_counts(sl,sm,sr,rr,par,cntid='rr')
        if par.estimator in ('HAM','DP','LS') : writeasc_counts(sl,sm,sr,dr,par,cntid='dr')
        # Write log entry now just for order  -------
        msg = '> COUNTS object saved in  : ' + par.outfn + '.cnt'
        log.info(msg)
    ...
    # Build ouput  ------------------------------------------------------------
    counts = buildoutput(par,npts=[npt,npt1],binslmr=sepsout,dd=dd,rr=rr,dr=dr,
                         bootc=bdd,cf=xis,cferr=xiserr)
...
def th_A(tab, par, nthreads=-1, write=True, plot=False , **kwargs):
    ...
    if plot:
        try:
            #plt.figure(1)
            plt.figure('Counts plot')
            plotcf(septout[1], dd, 0, fac=1., write=write, par=par, **kwargs)
        except ValueError:
            print('Warning: there is a problem with the plot !!!') 
    ...
    # Write output files  -----------------------------------------------------
    if write:
        writeasc_counts(*septout, dd, par, cntid='theta')
        savepars(par)
        savecounts(counts)
...
def th_C(tab, tab1, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    if par.autogrid:
        log.info('SK Autogrid ON')
        par.mxh1,par.mxh2,tdens = bestSKgrid2d(par,[npt,npt1],[tab[cra].data,tab1[cra1].data],dens=par.dens)
        log.info('SK cell target density'.ljust(lj)+' : {:0.3f}'.format(tdens))
    else:
        log.info('Autogrid OFF')
    log.info('SK grid size [dec,ra]'.ljust(lj)+' : ' + str([par.mxh1,par.mxh2]))
    ...
    if plot:
        try:
            #plt.figure(1)
            plt.figure('Counts plot')
            plotcf(septout[1],dr,0,fac=1.,write=write,par=par,**kwargs)
        except ValueError:
            print('Warning: there is a problem with the plot !!!') 
...
def acf(tab, tab1, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    if plot:
        try:
            ...
            plotcf(septout[1], wth, wtherr, fac=1., write=write, par=par, **kwargs)
        except ValueError:
            print('Warning: there is a problem with the plot !!!') 

    # Build ouput  ------------------------------------------------------------
    counts = buildoutput(par, npts=[npt,npt1], binslmr=septout, dd=dd, rr=rr, dr=dr,
                         bootc=bdd, cf=wth, cferr=wtherr)
    ...
    if write:
        ...
        if par.estimator not in ('DP'): writeasc_counts(*septout, rr, par, cntid='rr')
        if par.estimator in ('HAM','DP','LS') : writeasc_counts(*septout, dr, par, cntid='dr')
        savepars(par)
        savecounts(counts)
...
def accf(tab, tab1, tab2, par, nthreads=-1, write=True, plot=False, **kwargs):
    ...
    **cross** samples, this routine calculates the **two-point angular space
    ...
    # Initialize logs, check if par has the right kind, etc. Common to all CF functions
    (par,log,logf,logff,runspyder,t0) = initialize('accf', par, nthreads=nthreads, write=write, plot=plot)
    ...
    # Build ouput  ------------------------------------------------------------
    counts = buildoutput(par, npts=[npt,npt1,npt2], binslmr=septout, cd=cd, cr=cr,
                         bootc=bcd, cf=wth, cferr=wtherr)
...
# =============================================================================    
def buildoutputC(par, npts=[], binslmr=[], dd=None, dr=None, bootc=None,
                 intpi=None, intpib=None):
    """
    ...
    if par.kind in ['rppiA','rppiC']:         # save bins, intpi, etc.
        ...
        counts.intpi  = intpi
        if par.doboot : counts.intpib = intpib
    ...
    if iscross:                               # save counts
        counts.dr = dr
        if par.doboot: counts.bdr = bootc
    else:
        counts.dd = dd
        if par.doboot: counts.bdd = bootc
    return counts


# =============================================================================
def buildoutput(par, npts=[], binslmr=[], dd=None, rr=None, dr=None, cd=None, 
                cr=None, bootc=None, cf=None, cferr=None):
    """
    Given a set of pair count arrays, assemble the final **counts** output
    dictionary, adding also bins, input parameters and relevant sample sizes.
    ...
    if iscross:
        counts.cd = cd
        counts.cr = cr
        if par.doboot: counts.bcd = bootc
    else:
        counts.dd = dd
        if par.estimator not in ('DP'): counts.rr = rr
        if par.doboot: counts.bdd = bootc
        if par.estimator in ('HAM','DP','LS'): counts.dr = dr
