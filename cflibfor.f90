  ...
  ! 
  !  An example of python usage:
  !     > import cflibfor 
  !     > wbts = cflibfor.mod.bootstrap(npt,100)
  !
  ! HISTORY
  !  ---------------------------------------------------------------------
  !  2015-05-13: Written by ED
  !
  ! TODO
  !  ---------------------------------------------------------------------
  !  (X) Implement vmax
...
subroutine bootstrap(npt,nsamp,fixseed,w)
  !=======================================================================
  ! NAME
  !  bootstrap()
  !
  ...
  !
  ! DESCRIPTION
  !  Given npt particles and nsamp desired bootstrap
  !  within the area specified by (ral,rau,decl,decu,dcoml,dcomu).
  !
  ! INPUTS
  !  Variable----Type------------Description------------------------------
  !  npt         int             Number of particles
  !  nsamp       int             Number of bootstrap samples to return
  ...
  ! OUTPUTS
  !  Variable----Type----------------Description--------------------------
  !  w           real*4(npt,nsamp)   Bootstrap resampling weights
  !
  ! HISTORY  -------------------------------------------------------------
  !  2015-05-13: Written by ED
implicit none
integer      :: npt,nsamp,fixseed,n,ipt,isamp,isel
real(kind=4) :: sd
real(kind=4) :: w(nsamp,npt)
integer,allocatable :: seed(:)
...
do isamp=1,nsamp
   do ipt=1,npt
      ...
      w(isamp,isel) = w(isamp,isel) + 1.
   enddo
enddo

end subroutine bootstrap

subroutine bootstrapDP(npt,nsamp,fixseed,w)
  !=======================================================================
  ! NAME
  !  bootstrap()
  !
...
do isamp=1,nsamp
   do ipt=1,npt
      call random_number(sd)
      isel = floor(sd*(npt-1)) + 1
      w(isamp,isel) = w(isamp,isel) + 1.0d0
   enddo
enddo

end subroutine bootstrapDP
...
!==========================================================================================
!   COUNTING ROUTINES - 3D SPHERICAL
!==========================================================================================

subroutine skll3d(mxh1,mxh2,mxh3,np,ra,dec,r,sbound,sepv,nsepv,sk,ll)
!===============================================================================
! NAME
!  skll3d()
!
...
!  ra          real8(np)       RA of particles
!  dec         real8(np)       DEC of particles
!  r           real4(np)       Comoving distance to redshift of particles
...
implicit none
integer :: mxh1,mxh2,mxh3,np,nsepv
real(kind=8) :: ra(np),dec(np),sbound(6),ral,rau,decl,decu,dcoml,dcomu,hc1,hc2,hc3
real(kind=8) :: r(np),sepv(nsepv+1),rvmax  !kind=4
integer :: sk(mxh3,mxh2,mxh1),ll(np),q(3)
integer :: nc3,i,i0
...
do i=1, np
   ...
   ! Sort in increasing r(i) so that the sk element has the smallest r
   if(ll(i)==0) cycle
   if(r(i)<=r(ll(i))) cycle
   sk(q(3),q(2),q(1)) = ll(i)
   i0 = ll(i)
...
!call cpu_time(t1)
!write(*,*)'SK-LL construction time [s] = ',(t1-t0)
...
!
! DESCRIPTION
!  Count data pairs in a given skip cell (ixh1), i.e. a declination band, for
!  ONE sample of particles in (ra,dec,dcom) coordinates. Set ixh1<0 to loop over
!  all survey skip cells
!
! INPUTS
!  Variable---Type-----------Description----------------------------------------
!  ixh1       int            Skip cell index in which to compute the counts
!  npt        int            Number of particles
...
!  sepp       r8(nbs+1)      Bins in projected separation [Mpc/h]
!  nsepv      int            Number of radial separation bins
!  sepv       r8(nbs+1)      Bins in radial separation [Mpc/h]
...
!  all survey skip cells
!
!  This routine considers weighted pair counts (instead of just 1) and optionally
!  applies fiber-collision corrections
!
! INPUTS
!  Variable---Type-----------Description----------------------------------------
!  ixh1       int            Skip cell index in which to compute the counts
...
!  dec        r8(npt)        DEC of particles [deg]
!  dc         r8(npt)        DCOM of particles [Mpc/h]
!  wei        r4(npt)        Weights of particles
!  x,y,z      r8(npt)        X,Y,Z coordinates of particles (see radec2xyz())
!  nsepp      int            Number of projected separation bins
!  sepp       r8(nbs+1)      Bins in projected separation [Mpc/h]
!  nsepv      int            Number of radial separation bins
...
subroutine skll2d(mxh1,mxh2,np,ra,dec,sbound,sk,ll)
!===============================================================================
! NAME
!  skll2d()
!
...
!  ll          int(np)           Linked list for particles
!
! NOTES  -----------------------------------------------------------------------
!  1. Angles are in degrees
!  2. The boundaries of RA should be 0 - 360
...
implicit none
integer :: mxh1,mxh2,np,q(2)
real(kind=8) :: ra(np),dec(np),sbound(4),hc1,hc2
integer :: sk(mxh2,mxh1),ll(np),i,i0
...
do i=1, np
   ...
   ! Sort in increasing th(i) so that the head-of-chain element has the smallest th
   if(ll(i)==0) cycle
